...
- [DISTRIBUTED CACHE SYSTEM DESIGN](#distributed-cache-system-design)
  - [Building a High-Performance In-Memory Data Store](#building-a-high-performance-in-memory-data-store)
  ...
  - [4. DATABASE DESIGN](#4-database-design)
    ...
    - [Persistence Structures](#persistence-structures)
  - [5. API DESIGN](#5-api-design)
    - [Base Configuration](#base-configuration)
    - [Core String Operations](#core-string-operations)
      - [SET - Store a key-value pair](#set-store-a-key-value-pair)
      - [GET - Retrieve a value by key](#get-retrieve-a-value-by-key)
      - [DEL - Delete one or more keys](#del-delete-one-or-more-keys)
      - [MGET - Get multiple values](#mget-get-multiple-values)
      - [MSET - Set multiple key-value pairs](#mset-set-multiple-key-value-pairs)
    - [Expiration Operations](#expiration-operations)
      - [EXPIRE - Set key expiration](#expire-set-key-expiration)
      - [TTL - Get time to live](#ttl-get-time-to-live)
      - [PERSIST - Remove expiration](#persist-remove-expiration)
    - [Atomic Operations](#atomic-operations)
      - [INCR - Increment integer value](#incr-increment-integer-value)
      - [INCRBY - Increment by amount](#incrby-increment-by-amount)
      - [DECR / DECRBY - Decrement operations](#decr-decrby-decrement-operations)
    - [List Operations](#list-operations)
      - [LPUSH - Prepend to list](#lpush-prepend-to-list)
      - [RPUSH - Append to list](#rpush-append-to-list)
      - [LPOP - Remove and return first element](#lpop-remove-and-return-first-element)
      - [LRANGE - Get range of elements](#lrange-get-range-of-elements)
    - [Set Operations](#set-operations)
      - [SADD - Add members to set](#sadd-add-members-to-set)
      - [SMEMBERS - Get all set members](#smembers-get-all-set-members)
      - [SISMEMBER - Check membership](#sismember-check-membership)
    - [Hash Operations](#hash-operations)
      - [HSET - Set hash field](#hset-set-hash-field)
      - [HGET - Get hash field value](#hget-get-hash-field-value)
      - [HGETALL - Get all hash fields and values](#hgetall-get-all-hash-fields-and-values)
    - [Sorted Set Operations](#sorted-set-operations)
      - [ZADD - Add members with scores](#zadd-add-members-with-scores)
      - [ZRANGE - Get range by rank](#zrange-get-range-by-rank)
      - [ZRANK - Get member rank](#zrank-get-member-rank)
    - [Pattern Matching Operations](#pattern-matching-operations)
      - [KEYS - Find keys by pattern](#keys-find-keys-by-pattern)
      - [SCAN - Iterate keys incrementally](#scan-iterate-keys-incrementally)
    - [Pub/Sub Operations](#pubsub-operations)
      - [PUBLISH - Publish message to channel](#publish-publish-message-to-channel)
      - [SUBSCRIBE - Subscribe to channels](#subscribe-subscribe-to-channels)
      - [PSUBSCRIBE - Subscribe to patterns](#psubscribe-subscribe-to-patterns)
    - [Administrative Operations](#administrative-operations)
      - [INFO - Get server information](#info-get-server-information)
- [Memory](#memory)
      - [FLUSHDB - Clear current database](#flushdb-clear-current-database)
      - [CONFIG GET/SET - Get/set configuration](#config-getset-getset-configuration)
    - [Cross-Cutting Concerns](#cross-cutting-concerns)
    - [API Trade-Offs](#api-trade-offs)
      - [Decision: Binary Protocol (RESP) vs REST API](#decision-binary-protocol-resp-vs-rest-api)
      - [Decision: API Replication Strategy](#decision-api-replication-strategy)
      - [Decision: Single-threaded vs Multi-threaded Event Loop](#decision-single-threaded-vs-multi-threaded-event-loop)
      - [Decision: Text-based vs Binary Value Storage](#decision-text-based-vs-binary-value-storage)
  - [6. DEEP-DIVE COMPONENTS & TRADE-OFFS](#6-deep-dive-components-trade-offs)
    - [Component 1: Consistent Hashing for Sharding](#component-1-consistent-hashing-for-sharding)
      - [Decision: Consistent Hashing vs Hash Slot-based Sharding](#decision-consistent-hashing-vs-hash-slot-based-sharding)
      - [Decision: Number of Virtual Nodes (150-200 vs 1)](#decision-number-of-virtual-nodes-150-200-vs-1)
    - [Component 2: Replication and Failover](#component-2-replication-and-failover)
      - [Decision: Replication Mode Selection](#decision-replication-mode-selection)
      - [Decision: Sentinel vs Embedded Cluster Mode](#decision-sentinel-vs-embedded-cluster-mode)
      - [Decision: Automatic vs Manual Failover](#decision-automatic-vs-manual-failover)
    - [Component 3: Memory Management and Eviction](#component-3-memory-management-and-eviction)
      - [Decision: True LRU vs Approximate LRU](#decision-true-lru-vs-approximate-lru)
      - [Decision: Active vs Passive Expiration](#decision-active-vs-passive-expiration)
      - [Decision: Memory Limit Hard vs Soft](#decision-memory-limit-hard-vs-soft)
    - [Component 4: Persistence (Optional)](#component-4-persistence-optional)
      - [Decision: No Persistence vs RDB vs AOF vs Hybrid](#decision-no-persistence-vs-rdb-vs-aof-vs-hybrid)
      - [Decision: Fork vs Thread-based Persistence](#decision-fork-vs-thread-based-persistence)
    - [Caching Strategy](#caching-strategy)
  - [7. BOTTLENECKS & IMPROVEMENTS](#7-bottlenecks-improvements)
    - [Potential Bottlenecks](#potential-bottlenecks)
      - [Bottleneck 1: Single Master Write Bottleneck](#bottleneck-1-single-master-write-bottleneck)
      - [Bottleneck 2: Network Bandwidth Saturation](#bottleneck-2-network-bandwidth-saturation)
      - [Bottleneck 3: Memory Fragmentation](#bottleneck-3-memory-fragmentation)
      - [Bottleneck 4: Hot Key Problem](#bottleneck-4-hot-key-problem)
      - [Bottleneck 5: Large Key Problem](#bottleneck-5-large-key-problem)
    ...
    - [Future Enhancements](#future-enhancements)
      ...
      - [3. Operational Improvements](#3-operational-improvements)
